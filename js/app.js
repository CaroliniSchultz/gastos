...
document.getElementById('btn-select-all-tx').addEventListener('click', () => {
  getFilteredTransactions().forEach((t) => selectedTxIds.add(t.id));
  renderTransactions();
});
...
document.getElementById('btn-delete-selected').addEventListener('click', async () => {
  ...
  if (!count) return;
  if (!confirm(`Excluir ${count} lançamento(s) selecionado(s)? Essa ação não pode ser desfeita.`)) return;
  try {
    const batch = writeBatch(db);
    selectedTxIds.forEach((id) => batch.delete(doc(db, 'users', currentUser.uid, 'transactions', id)));
    await batch.commit();
    selectedTxIds.clear();
...
function renderTransactions() {
  ...
  selectedTxIds.forEach((id) => { if (!validIds.has(id)) selectedTxIds.delete(id); });

  const bar = document.getElementById('bulk-actions-bar');
  if (selectedTxIds.size > 0) {
    bar.style.display = 'flex';
    document.getElementById('bulk-actions-count').textContent = `${selectedTxIds.size} selecionado(s)`;
  } else {
  ...
  container.innerHTML = list.map((t) => {
    ...
    return `
      <div class="ledger-row">
        <input type="checkbox" class="tx-checkbox" data-select="${t.id}" ${checked} aria-label="Selecionar lançamento" style="margin-right:4px;">
  ...
  container.querySelectorAll('[data-select]').forEach((cb) => cb.addEventListener('change', () => {
    if (cb.checked) selectedTxIds.add(cb.dataset.select); else selectedTxIds.delete(cb.dataset.select);
    renderTransactions();
  }));
}
